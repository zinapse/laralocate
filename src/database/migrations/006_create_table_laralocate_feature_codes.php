...
{
    private array $feature_codes = [
        'A' => [
            'examples' => ['country', 'state', 'region'],
            'codes' => [
    ...
    public function up()
    {
        Schema::create('laralocate_feature_codes', function (Blueprint $table) {
            $table->id();
            $table->string('code');
        ...
        // Iterate through the feature_codes variable to populate the table
        foreach($this->feature_codes as $code => $data) {
            // If there are no examples or codes then skip this row
            if(empty($data['examples']) || empty($data['codes'])) continue;

            // Set the examples variable
            $examples = implode('|', $data['examples']);

            // Create the toplevel record
            $record = new FeatureCode;
            $record->code = $code;
            $record->short_desc = $examples;
            $record->save();

            // Set the subcodes array
            $subcodes = $data['codes'];

            // Iterate through the subcodes
            foreach($subcodes as $subcode => $desc) {
                // Create the sublevel record
                $subrecord = new FeatureCode;
                $subrecord->code = $subcode;

                // Get the descriptions by splitting the $desc string
                $descriptions = explode(' - ', $desc);
                if(empty($descriptions[0])) continue;

                // Populate the record's variables and save it
                $subrecord->short_desc = $descriptions[0];
                $subrecord->long_desc = $descriptions[1] ?? null;
                $subrecord->parent_id = $record->id;
                $subrecord->save();
            }
    ...
    {
        // Drop the table
        Schema::dropIfExists('laralocate_feature_codes');
    }
};